...
export default React.memo(({ prefix, dataSource, data, dataChange }) => {
  ...
  return (
    <div className={`${currentPrefix}-datatype ${currentPrefix}-form`}>
      ...
      {dataTypeSupport.map(d => (
        <div className={`${currentPrefix}-form-item`} key={d.id}>
          <span className={`${currentPrefix}-form-item-label`} title={d.defKey}>
            {`${d.defKey}${
              db === d.id
                ? FormatMessage.string({ id: 'dataType.defaultDb' })
                : ''
            }`}
          </span>
          <span className={`${currentPrefix}-form-item-component`}>
