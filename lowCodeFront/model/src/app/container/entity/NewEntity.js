...
export default React.memo(({prefix, dataSource, dataChange, isNewView,
  ...
  return (
    <div className={`${currentPrefix}-new-entity`}>
      <div className={`${currentPrefix}-form`}>
        <div className={`${currentPrefix}-form-item`}>
          <span
            className={`${currentPrefix}-form-item-label`}
            title={FormatMessage.string({
              id: `${!isNewView ? 'tableBase' : 'viewBase'}.defKey`,
            })}
          >
            <span>
              <FormatMessage
          ...
          <span className={`${currentPrefix}-form-item-component`}>
            <Input
              ...
              placeholder={FormatMessage.string({
                id: `${
                  !isNewView ? 'tableBase.tableName' : 'viewBase.viewName'
                }`,
              })}
        ...
        <div className={`${currentPrefix}-form-item`}>
          <span
            className={`${currentPrefix}-form-item-label`}
            title={FormatMessage.string({
              id: `${!isNewView ? 'tableBase' : 'viewBase'}.defName`,
            })}
          >
            <FormatMessage
              id={`${!isNewView ? 'tableBase' : 'viewBase'}.defName`}
          ...
          <span className={`${currentPrefix}-form-item-component`}>
            <Input
              ...
              placeholder={FormatMessage.string({
                id: `${
                  !isNewView ? 'tableBase.tableTitle' : 'viewBase.viewTitle'
                }`,
              })}
            ...
            <span className={`${currentPrefix}-form-item-component`}>
              <Select defaultValue="1" onChange={onChange} notAllowEmpty>
                <Select.Option value="1" key="1">
                  {FormatMessage.string({ id: 'tableBase.empty' })}
                </Select.Option>
                <Select.Option value="2" key="2">
                  {FormatMessage.string({ id: 'tableBase.extend' })}
                </Select.Option>
              </Select>
            </span>
          </div>
            ...
            <div className={`${currentPrefix}-form-item`}>
              ...
              <span className={`${currentPrefix}-form-item-component`}>
                <a onClick={pickerFields}>
                  <FormatMessage
                    id="tableBase.parentFieldsCount"
                    data={{ count: extendFields.length }}
                  />
                </a>
              </span>
            </div>
          ...
          <span className={`${currentPrefix}-form-item-component`}>
            ...
            >
              {dataSource?.viewGroups?.map(v => (
                <Option key={v.id} value={v.id}>{`${v.defKey}(${
                  v.defName || v.defKey
                })`}</Option>
              ))}
            </MultipleSelect>
