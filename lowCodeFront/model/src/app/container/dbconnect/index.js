...
import { pingLoadDriver } from '../../../api/project';
...
export default React.memo(({prefix, dataSource, config, dataChange, lang}) => {
      ...
      if (d.defKey === key) {
        ...
        return {
          ...
          properties: name === 'type' ? {
            ...
            url1: url[dataTypeSupport.defKey?.toLocaleLowerCase()]?.url || '',
            password: '',
            username: '',
          } : d.properties,
        };
  ...
  const test = useCallback((e, btn) => {
    // 加载驱动测试
    // pingLoadDriver(propertiesRef.current).then((response) => {
      ...
      Object.keys(url).map((o) => {
        return (
          <div key={o} className={`${currentPrefix}-dbconnect-example-config-item`}>
            ...
            <div className={`${currentPrefix}-dbconnect-example-config-item-content-label`}>
              url1:
              <span className={`${currentPrefix}-dbconnect-example-config-item-content-value`}>{url[o].url1}</span>
            </div>
          </div>
        );
      })
    ...
    </div>
    {
      defaultConn && <div className={`${currentPrefix}-dbconnect-right`}>
        ...
        <div className={`${currentPrefix}-form-item`}>
          <span
            className={`${currentPrefix}-form-item-label`}
            title={FormatMessage.string({id: 'dbConnect.url1'})}
          >
            <span className={`${currentPrefix}-form-item-label-require`}>{}</span>
          ...
          <span className={`${currentPrefix}-form-item-component`}>
            <Input
              onChange={e => onChange(e, 'url1')}
              value={properties.url1 || ''}
            />
          </span>
        </div>
        <div className={`${currentPrefix}-form-item`}>
