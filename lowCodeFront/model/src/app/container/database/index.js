...
import { SimpleTab, FormatMessage, Input, Radio, Checkbox } from 'components';
...
export default React.memo(({ prefix, data = {}, dataChange, dataSource }) => {
  const dataTypeSupport = dataSource?.profile?.dataTypeSupports?.filter(
    d => d.id === data.applyFor
  )[0]?.defKey;
  ...
  return (
    <div className={`${currentPrefix}-database-container`}>
      ...
      <div className={`${currentPrefix}-form-item`}>
        ...
        <span className={`${currentPrefix}-form-item-component`}>
          <Checkbox disable defaultChecked={data.isDefault} />
        </span>
      </div>
      <div className={`${currentPrefix}-form-item`}>
        <span
