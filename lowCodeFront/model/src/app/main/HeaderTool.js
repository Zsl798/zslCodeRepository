...
import {FormatMessage, GroupIcon, Icon, SearchSuggest, Slider, NumberInput, Modal, Button} from 'components';
...
export default React.memo(forwardRef(({currentPrefix, close, iconClick, colorChange, openModal,
  ...
  return <div className={`${currentPrefix}-head`}>
    ...
    <GroupIconGroup>
      ...
      <GroupIcon
        ...
        dropMenu={[
            ...
            {key: 'word', name: FormatMessage.string({id: 'toolbar.exportWord'}),disable: activeTab?.type !== 'diagram'},
            {
              key: 'img',
    ...
    <div className={`${currentPrefix}-head-button`}>
      <Button style={{}} onClick={() => iconClick(null, 'check')}>
        <FormatMessage id='button.check'/>
      </Button>
    </div>
  </div>;
}));
