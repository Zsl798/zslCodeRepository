    ...
    if (process.platform === 'darwin') {
      ...
      if (current?.getAttribute('class').includes('max')) {
        ...
        current.setAttribute(
          'class',
          `${currentPrefix}-toolbar-opt-darwin-min`,
        );
      } else {
        // maximize(false);
        current.setAttribute(
          'class',
          `${currentPrefix}-toolbar-opt-darwin-max`,
        );
      }
    } else if (current?.getAttribute('class').includes('fa-window-maximize')) {
      // maximize(true);
  ...
  if (platform === 'json') {
    const darwinClass =
      process.platform === 'darwin'
        ? ` ${currentPrefix}-toolbar-title-darwin`
        : '';
    return (
      <div
        className={`${currentPrefix}-toolbar`}
        onDoubleClick={titleBarDoubleClick}
      >
        <span className={`${currentPrefix}-toolbar-title${darwinClass}`}>
          <span>{}</span>
          <span>{title}</span>
          <span className={`${currentPrefix}-toolbar-info`} title={info}>
            {info}
          </span>
        </span>
        {process.platform !== 'darwin' ? (
          ...
          <span className={`${currentPrefix}-toolbar-opt-darwin`}>
            <span onClick={_close}>
              <Icon type="fa-times" />
            </span>
            {/* <span onClick={minimize}><Icon type='fa-minus'/></span> */}
            <span
              className={`${currentPrefix}-toolbar-opt-darwin-max`}
              ref={iconRef}
              onClick={fullScreenClick}
            >
              {}
