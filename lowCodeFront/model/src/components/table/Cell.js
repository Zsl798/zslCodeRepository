...
export default React.memo(({f, name, remarkChange, onKeyDown, currentPrefix,
  ...
  const ditems = () =>{
      ...
      for(var i = 0;i < dicts.length;i++){
       var items = dicts[i].items;
        for(var j = 0;j < items.length;j++){
          ditemArr.push(items[j]);
        }
      };
      return ditemArr;
  };
  const onChangeSetItem = (values) =>{
        debugger;
        onChange(values);
        let dicId = values.target.value;
    for(var i = 0;i < dicts.length;i++){
      if(dicts[i].id === dicId){
        ditemArr = dicts[i].items;
      }
    }
  };
...
//   })}
  ...
  if (reading) {
    ...
    />;
  } else if (name === 'refCodeTable') {
    return <div className={`${currentPrefix}-table-dict`}>
      <Component.MultipleSelect simple>
        {
          ditems().map(v => (
    ...
    return <div className={`${currentPrefix}-table-dict`}>
      <Component.MultipleSelect
        dropdownRender={menus => <div>
          ...
          // onChange={values => onChangeSetItem({
          //   target: {
          //     value: values[0] || '',
          //   },
          // })}
