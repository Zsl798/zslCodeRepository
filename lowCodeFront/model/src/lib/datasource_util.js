...
export const allType = [
 ...
 /* { type: 'mainTable', name: 'mainTables', defKey: 'defKey' },
  { type: 'sourceTable', name: 'sourceTables', defKey: 'defKey' },*/
];
...
export const getFullColumns = () => {
  return [
    ...
    {code: 'refCodeTable', value: FormatMessage.string({id: 'tableHeaders.refCodeTable'}), newCode: 'refCodeTable', com: 'SearchSelect', relationNoShow: true},
...
export const transform = (f, dataSource, code, type = 'id') => {
  ...
  //主数据
  //const mainTables = dataSource?.mainTables || [];
  //源数据
  //const sourceTables = dataSource?.sourceTables || [];
...
export const transformationData = (oldDataSource) => {
  ...
  if (oldDataSource.version === '3.0.0') {
    ...
    tempDataSource = {
      ...
      //mainTables: (tempDataSource.mainTables || []).map(m => refactor(m)),
      //sourceTables: (tempDataSource.sourceTables || []).map(s => refactor(s)),
    };
  }
  // 2.处理新增的列
  ...
  if (compareVersion('3.5.2', oldDataSource.version.split('.'))) {
    ...
    //mainTables: (tempDataSource.mainTables || []).map(d => updateHeaders(d, 'mainTable')),
    //sourceTables: (tempDataSource.sourceTables || []).map(d => updateHeaders(d, 'sourceTable')),
