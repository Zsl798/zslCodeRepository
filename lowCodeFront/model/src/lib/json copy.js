...
export const getUserConfig = () => {
  return new Promise((res, rej) => {
    ...
    // Promise.all([
    //   getData(userConfigPath, defaultUserConfigData),
    //   getData(projectConfigPath, defaultProjectConfigData),
    // ])
    //   .then((result) => {
    //     setTimeout(() => {
    //       res(result);
    ...
    dialog
      ...
      .then(({ filePath }) => {
        if (filePath) {
          ...
          saveNormalFile(tempFile, refactor ? refactor(data, tempFile) : data)
            .then((data) => {
              res({
                data,
                filePath: tempFile,
...
export const openProjectFilePath = (errorFileMessage, suffix) => {
  ...
  return new Promise((res, rej) => {
    openFileOrDirPath([{ name: tempSuffix, extensions: extensions }])
      .then((file) => {
        ...
        Promise.all(
          ...
          .then((results) => {
            ...
            res(
              results.sort(
                (a, b) =>
                  moment(a?.date, format).millisecond() -
                  moment(b?.date, format).millisecond()
              )
            );
          })
          .catch((err) => {
...
export const connectDB = (dataSource, config, params = {}, cmd, cb) => {
  ...
  const javaHome =
    config?.javaHome || _.get(dataSource, 'profile.javaHome', '');
  const jvm = 'jvm' in (config || {}) ? config.jvm : defaultJVM;
  const jar = ipcRenderer.sendSync('jarPath');
  const tempValue = javaHome
    ? `${javaHome}${path.sep}bin${path.sep}java`
    : 'java';
    ...
    Promise.all(
      images.map((i) => {
        const filePath =
          userConfigPath +
          path.sep +
          (i.group ? `${i.group}-${i.fileName}` : i.fileName) +
          '.png';
...
export const saveAsWordTemplate = () => {
  return copyFile(getDefaultWordTemplate(), [
    { name: 'chiner-docx-tpl', extensions: ['docx'] },
  ]);
};
...
export const selectWordFile = (dataSource) => {
  ...
  return new Promise((res, rej) => {
    openFileOrDirPath([], ['openDirectory']).then((dir) => {
      res([
        `${dir}${path.sep}${name}-${moment().format('YYYYMDHHmmss')}.docx`,
        defaultPath,
      ]);
    });
  });
...
export const writeLog = (err) => {
  const logPath = `${basePath}${path.sep}${moment().format(
    'YYYY-M-D-HH-mm-ss'
  )}-error-log.txt`;
  return new Promise((res) => {
    saveNormalFile(logPath, err.stack).then(() => res(logPath));
  });
};
  ...
  if (core.info && config.autoBackup) {
    ...
    fs.readdir(dir, (error, files) => {
      if (!error) {
        ...
        const fileName = `${core.data.name}-backup-${moment().format(
          'YYYYMDHHmmss'
        )}.chnr.json`;
        fs.writeFile(
          path.join(dir, fileName),
          JSON.stringify(core.data, null, 2),
          () => {
            callback && callback();
          }
        );
