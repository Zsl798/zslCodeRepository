...
// } from '../../lib/middle';
import { saveJsonPromise, saveJsonPromiseAs } from '../../lib/middle';
...
// import { saveJsonPromise, dirSplicing } from '../../lib/json';
...
const readProjectSuccess = (data, versionsData, id, isDemoProject) => {
  return {
...
// export const readProject = (id, title, getState, type, isDemoProject) => {
//   return (dispatch) => {
// dispatch(openLoading(title)); // 开启全局loading
...
export const readProject = (id, title, getState, type, isDemoProject) => {
  return (dispatch) => {
    // loadItemById({ projectId: id })
    loadItemById(id).then((projectData) => {
      console.log('projectData', projectData);
      if (!isDemoProject) {
        debugger;
        const newData = transformationData(JSON.parse(projectData.content));
        setMemoryCache('data', newData);
        dispatch(readProjectSuccess(newData, [], id)); //undo
        dispatch(closeLoading(STATUS[1], null, '', type));
        // queryItemList()
        //   .then((listInfo) => {
        //     addHistory(listInfo, (err) => {
        //       if (!err) {
        //         setMemoryCache('data', newData);
        //         dispatch(readProjectSuccess(newData, [], id)); //undo
        //         dispatch(closeLoading(STATUS[1], null, '', type));
...
export const openProject = (title, type, id, suffix, isDemoProject) => {
  // 从系统中选择项目 无需传递路径
  return (dispatch, getState) => {
    if (id) {
      readProject(id, title, getState, type)(dispatch);
    } else {
      dispatch(readProjectFail(err));
      // const config = getState()?.config?.data[0];
...
export const createProject = (data, title, type) => {
  ...
  return (dispatch, getState) => {
   // dispatch(openLoading(title));
    ...
    new Promise((res, rej) => {
      ...
      // saveDataModel(param).then((itemId) => {
      //   debugger;
      ...
      .then((itemId) => {
        ...
        //   id: itemId,
        // };
        // // saveJsonPromise(newData)
        // const param = {
        //   projectId: itemId,
        //   content: JSON.stringify(newData),
        // };
        // saveDataModel(param).then((response) => {
        //   debugger;
        //   console.log('save successfully!', response);
          // const params = {
          //   projectName: '',
          //   pageNo: 1,
          //   pageSize: getState()?.config?.data[0].pageSize,
          // };
          ...
          //       ...params,
          //     };
          //     addHistory(projectListInfo, (err) => {
          //       if (!err) {
          //         dispatch(createProjectSuccess(newData));
          //         dispatch(closeLoading(STATUS[1], null, '', type));
          //       } else {
...
export const renameProject = (listInfo, title) => {
  return (dispatch, getState) => {
    dispatch(openLoading(title));
    updateHistory(listInfo)(dispatch, getState);
  };
};

export const removeProject = (data, title) => {
  return (dispatch, getState) => {
    deleteItem({ projectId: data.projectId }).then((response) => {
      ...
      queryItemList(params).then((listInfo) => {
        ...
        updateHistory(newListInfo)(dispatch, getState);
      });
    });
  };
};
